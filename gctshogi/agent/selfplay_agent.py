...
class SelfPlayAgentGroup:
    ...
    def __init__(self, model, positions_train, policy_value_batch_maxsize, checkpoint, playouts):
        # モデル
        self.model = model
        ...
        self.features = np.empty((self.policy_value_batch_maxsize, 104, 9, 9), dtype=np.float32)
    ...
    def eval_node(self):
        ...
        eval_features = self.features[0:self.current_policy_value_batch_index]
        x = np.array(eval_features, dtype=np.float32)
        logits, values = self.model.predict(x)
        ...
        for i, (logit, value) in enumerate(zip(logits, values)):
            ...
            for j in range(child_num):
                ...
                move_label = make_output_label(move, self.board.turn)
    ...
    def queuing_node(self, board, moves, repetitions, node):
        ...
        self.features[self.current_policy_value_batch_index] = make_input_features_from_board(board)
...
class SelfPlayAgent:
    ...
    def add_teacher(self, current_root_node):
        ...
        self.chunk.append((self.board.sfen(), self.board.move_number, legal_moves, visits))
    ...
    def next_step(self):
        ...
        if self.interruption_check():
            ...
            if __debug__ : logging.debug('id:{} ply:{} sfen {} move:{} value:{:.2f}'.format(
                ...
                self.id, self.board.move_number, self.board.sfen(), best_move, best_wp))
    ...
    def next_game(self, game_result):
        ...
        if __debug__ : logging.debug('id:{} ply:{} result:{}'.format(self.id, len(self.moves), game_result))
        ...
        if len(self.chunk) > 0:
            ...
            self.positions_train = (self.features, self.moves, game_result)
